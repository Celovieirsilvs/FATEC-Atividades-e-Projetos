...
INSERT INTO equipamento VALUES ( default, 'HPE ProLiant ML30 Gen10 Server', 'HP5412A4', '10/03/2022' ,
		'765WHP', 9199.99, 36, 'COMPUTADOR', 3, 5, 'ATIVO') ; --2029
...
INSERT INTO computador VALUES ( 2031, 'Xeon E-2224', 3.4, 4000, 64, 'SERVIDOR') ;
-- periferico
...
/* Atividade 09:
a) Mostrar para cada fornecedor (razão social) o valor total 
acumulado de equipamentos fornecidos por mês de aquisição; */
...
GROUP BY d.nome_depto
HAVING COUNT(ae.num_patr_eqpto) >= 2 ;
...
FROM equipamento eq INNER JOIN computador c
ON ( eq.num_patrimonio = c.num_patr_comp )
WHERE eq.num_patrimonio IN ( SELECT c.num_patr_comp
                               FROM computador c
...
-- OUTER JOIN
SELECT e.*, c.*, temperif.computador
FROM equipamento e JOIN computador c 
    ON (e.num_patrimonio = c.num_patr_comp)
LEFT JOIN  ( 
             SELECT num_patr_comp_aloc AS computador
             FROM perifericos_instalados
             WHERE dt_term_aloc IS NULL ) temperif
ON ( temperif.computador = c.num_patr_comp) 
WHERE temperif.computador IS NULL ;

-- NOT IN
SELECT eq.*, c.*
FROM equipamento eq, computador c
WHERE eq.num_patrimonio = c.num_patr_comp
AND c.num_patr_comp  NOT IN
                     ( SELECT num_patr_comp_aloc
                       FROM perifericos_instalados
                       WHERE dt_term_aloc IS NULL)
ORDER BY 1;
...
FROM equipamento eq, computador c
WHERE eq.num_patrimonio = c.num_patr_comp
AND c.num_patr_comp IN
(
  SELECT num_patr_comp
  FROM computador
  EXCEPT
     (SELECT num_patr_comp_aloc
       FROM perifericos_instalados
...
/* b)	Montar uma lista com todos os dados dos periféricos que ainda não foram alocados a um computador.
Resolva com junção externa usando junção à esquerda e depois à direita.*/
-- esquerda
SELECT eq.*,  p.*, pi.num_patr_perif_aloc
FROM equipamento eq INNER JOIN periferico p
     ON (eq.num_patrimonio = p.num_patr_perif)
	 LEFT OUTER JOIN perifericos_instalados pi
	 ON (pi.num_patr_perif_aloc = p.num_patr_perif)
WHERE pi.num_patr_perif_aloc IS NULL ;
-- direita
SELECT eq.*,  p.*, pi.num_patr_perif_aloc
...
/* c)	Mostrar os dados dos computadores : Patrimonio-Modelo-Processador-Tipo Computador
que nunca teve software do tipo ‘Ferramenta CASE’ instalado. Resolva de três formas diferentes,
sendo uma delas com JUNÇÃO EXTERNA; */
SELECT * FROM instalacao_softw ;
-- junção externa
SELECT c.num_patr_comp,eq.modelo_eqpto, c.tipo_processador, c.tipo_computador,temcase.comptem 
FROM computador c LEFT OUTER JOIN 
     (SELECT isw.num_patr_comp AS comptem
       FROM instalacao_softw isw JOIN software sw
       ON ( sw.cod_softw = isw.cod_softw)
       WHERE UPPER(sw.tipo_softw) LIKE '%CASE%' ) temcase
ON ( c.num_patr_comp = temcase.comptem)
JOIN equipamento eq ON ( eq.num_patrimonio = c.num_patr_comp )
WHERE temcase.comptem IS NULL ;
-- NOT IN
SELECT c.num_patr_comp, eq.modelo_eqpto, c.tipo_processador, c.tipo_computador
FROM equipamento eq, computador c
WHERE eq.num_patrimonio = c.num_patr_comp
AND c.num_patr_comp NOT IN
(SELECT isw.num_patr_comp
       FROM instalacao_softw isw JOIN software sw
       ON ( sw.cod_softw = isw.cod_softw)
       WHERE UPPER(sw.tipo_softw) LIKE '%CASE%' ) ;
-- diferença
SELECT c.num_patr_comp, eq.modelo_eqpto, c.tipo_processador, c.tipo_computador
FROM equipamento eq, computador c
WHERE eq.num_patrimonio = c.num_patr_comp
AND c.num_patr_comp IN
(
  SELECT num_patr_comp FROM computador
  EXCEPT
        (SELECT isw.num_patr_comp
         FROM instalacao_softw isw JOIN software sw
         ON ( sw.cod_softw = isw.cod_softw)
...
E que não tiveram seus produtos instalados em servidores nos últimos 45 dias.
Resolva de três formas diferentes, sendo uma delas com JUNÇÃO EXTERNA. */
-- junção externa
SELECT e.*, sw.*, f.tipo_fabr, temsw45.fabrsw
FROM empresa e JOIN fabricante f
ON ( e.cod_empresa = f.cod_fabr)
JOIN software sw
ON ( f.cod_fabr = sw.cod_fabr_softw )
LEFT JOIN 
( SELECT sw.cod_fabr_softw AS fabrsw
       FROM instalacao_softw isw JOIN software sw
       ON ( sw.cod_softw = isw.cod_softw)
	   JOIN computador c
	   ON (c.num_patr_comp = isw.num_patr_comp)
       WHERE isw.dthora_inst > current_date - 45
	   AND c.tipo_computador = 'SERVIDOR') temsw45
ON ( f.cod_fabr = temsw45.fabrsw )
WHERE temsw45.fabrsw IS NULL ;
-- NOT IN
SELECT e.*, sw.*
FROM empresa e JOIN fabricante f
                   ON ( e.cod_empresa = f.cod_fabr)
               JOIN software sw
                   ON ( f.cod_fabr = sw.cod_fabr_softw )
AND f.cod_fabr NOT IN 
      (SELECT sw.cod_fabr_softw
       FROM instalacao_softw isw JOIN software sw
       ON ( sw.cod_softw = isw.cod_softw)
	   JOIN computador c
	   ON (c.num_patr_comp = isw.num_patr_comp)
       WHERE isw.dthora_inst > current_date - 45
	   AND c.tipo_computador = 'SERVIDOR') ;
...
FROM empresa e JOIN fabricante f
               ...
               JOIN software sw
                   ON ( f.cod_fabr = sw.cod_fabr_softw )
AND f.cod_fabr IN 
(SELECT cod_fabr FROM fabricante
EXCEPT
      (SELECT sw.cod_fabr_softw
       FROM instalacao_softw isw JOIN software sw
       ON ( sw.cod_softw = isw.cod_softw)
