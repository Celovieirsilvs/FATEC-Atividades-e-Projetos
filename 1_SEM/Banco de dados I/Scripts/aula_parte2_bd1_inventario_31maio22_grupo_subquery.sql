...
EXTRACT (HOUR FROM (ae.dt_term_alocacao - ae.dt_ini_alocacao )) As Dias
...
INSERT INTO alocacao_eqpto VALUES ( 2030, 1, current_timestamp - INTERVAL '19 DAYS', 
...
FROM equipamento
GROUP BY tipo_eqpto ;
--22) Mostrar o total de equipamentos por fabricante
SELECT e.rz_social AS Fabricante, SUM(eq.valor_aquisicao_eqpto) AS "Valor Total",
COUNT(eq.num_patrimonio) AS Qtde_eqptos
FROM equipamento eq JOIN fabricante fa
ON ( eq.cod_fabr = fa.cod_fabr)
JOIN empresa e ON (fa.cod_fabr = e.cod_empresa)
GROUP BY e.rz_social ;

SELECT eq.tipo_eqpto, COUNT(eq.num_patrimonio) AS Qtde_eqptos, 
       COUNT(eq.valor_aquisicao_eqpto) AS Qtde_valores_preenchidos
FROM equipamento eq
GROUP BY eq.tipo_eqpto;

SELECT COUNT(eq.num_patrimonio) AS Qtde_eqptos, 
       COUNT(eq.valor_aquisicao_eqpto) AS Qtde_valores_preenchidos
FROM equipamento eq ;
--23) Mostrar o total de equipamentos por fornecedor
SELECT e.rz_social AS Fornecedor, SUM(eq.valor_aquisicao_eqpto) AS "Valor Total",
COUNT(eq.num_patrimonio) AS Qtde_eqptos
FROM equipamento eq JOIN empresa e ON (eq.cod_forn = e.cod_empresa)
GROUP BY e.rz_social ;
--24) Total por fabricante e mês de aquisicao
SELECT e.rz_social AS Fabricante, 
TO_CHAR(eq.dt_aquisicao_eqpto, 'MM/YYYY') AS Mês_Aquisição,
SUM(eq.valor_aquisicao_eqpto) AS "Valor Total",
COUNT(eq.num_patrimonio) AS Qtde_eqptos
FROM equipamento eq JOIN fabricante fa
ON ( eq.cod_fabr = fa.cod_fabr)
JOIN empresa e ON (fa.cod_fabr = e.cod_empresa)
...
ORDER BY 1, 2 DESC;
--25) tempo medio de permanencia de alocação de eqpto a departamento
SELECT d.nome_depto AS Departamento,
ROUND(AVG(EXTRACT(DAY FROM (COALESCE(ae.dt_term_alocacao,current_date)
					           - ae.dt_ini_alocacao))), 0)
FROM alocacao_eqpto ae JOIN departamento d
ON (ae.cod_depto = d.cod_depto )
GROUP BY d.nome_depto ;

-- 26) Mostrar os computadores com mais de 3 softwares instalados
SELECT isw.num_patr_comp, eq.modelo_eqpto, COUNT(isw.cod_softw) As "Qtde instalados"
FROM instalacao_softw isw JOIN computador c
     ON ( isw.num_patr_comp = c.num_patr_comp)
        JOIN equipamento eq 
	 ON ( eq.num_patrimonio = c.num_patr_comp)
WHERE isw.dthora_desinst IS NULL -- antes do group by
GROUP BY isw.num_patr_comp, eq.modelo_eqpto
HAVING COUNT(isw.cod_softw) > 1 ;-- having sempre depois do group by

SELECT * FROM instalacao_softw ;
--27) Mostrar os fabricantes com mais de 5000 em equipamentos
SELECT e.rz_social AS Fabricante, SUM(eq.valor_aquisicao_eqpto) AS "Valor Total",
COUNT(eq.num_patrimonio) AS Qtde_eqptos
FROM equipamento eq JOIN fabricante fa
ON ( eq.cod_fabr = fa.cod_fabr)
JOIN empresa e ON (fa.cod_fabr = e.cod_empresa)
WHERE EXTRACT(YEAR FROM eq.dt_aquisicao_eqpto) = EXTRACT (YEAR FROM current_date)
GROUP BY e.rz_social
HAVING SUM(eq.valor_aquisicao_eqpto) > 5000 
AND COUNT(eq.num_patrimonio) = 1;
-- 29) Consultas aninhadas - Subconsulta / Subquery
-- Qual o computador menos rápido -> menor velocidade do processador
SELECT c.num_patr_comp, c.velocidade_proc_ghz
FROM computador c
ORDER BY 2 ASC
LIMIT 1 ;
-- melhor forma que traz todos que eventualmente empatam
SELECT c.* FROM computador c
WHERE c.velocidade_proc_ghz = ( SELECT MIN( c.velocidade_proc_ghz) 
							    FROM computador c ) ; 
-- 30) Mostrar todos os fabricantes de software que fabricam o mesmo tipo que o Windows 11
SELECT tipo_softw
FROM software
WHERE UPPER(nome_softw) LIKE '%WINDOWS%'
AND UPPER(versao_softw) LIKE '%11%';
SELECT * FROM software ;
UPDATE software SET versao_softw = 'R11.02' WHERE cod_softw = 'WIN11' ;
INSERT INTO software VALUES ( 'UBU21', 'Linux Ubuntu', 'R21', 'SISOPER',1);
SELECT e.cod_empresa,e.rz_social AS Fabricante_Softw, sw.tipo_softw,sw.nome_softw
FROM empresa e, fabricante fa, software sw
WHERE e.cod_empresa = fa.cod_fabr
AND fa.cod_fabr = sw.cod_fabr_softw
AND sw.tipo_softw = (SELECT tipo_softw
                     FROM software
                     WHERE UPPER(nome_softw) LIKE '%WINDOWS%'
                      AND UPPER(versao_softw) LIKE '%11%')
AND e.cod_empresa <> (  SELECT cod_fabr_softw
                        FROM software
                        WHERE UPPER(nome_softw) LIKE '%WINDOWS%'
                        AND UPPER(versao_softw) LIKE '%11%' );
